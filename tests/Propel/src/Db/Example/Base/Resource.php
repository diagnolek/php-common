...
abstract class Resource implements ActiveRecordInterface
{
     ...
     * @var string
     */
    public const TABLE_MAP = '\\Db\\Example\\Map\\ResourceTableMap';
    ...
    /**
     * Initializes internal state of Db\Example\Base\Resource object.
     */
    ...
    public function setUid($v)
    {
        ...
        if ($this->uid !== $v) {
            $this->uid = $v;
            $this->modifiedColumns[ResourceTableMap::COL_UID] = true;
        }
    ...
    public function hydrate(array $row, int $startcol = 0, bool $rehydrate = false, string $indexType = TableMap::TYPE_NUM): int
    {
        try {
            ...
            $col = $row[TableMap::TYPE_NUM == $indexType ? 0 + $startcol : ResourceTableMap::translateFieldName('Uid', TableMap::TYPE_PHPNAME, $indexType)];
            ...
            throw new PropelException(sprintf('Error populating %s object', '\\Db\\Example\\Resource'), 0, $e);
        }
    }
        ...
        if (!$this->alreadyInSave) {
            ...
            if ($this->reservationsScheduledForDeletion !== null) {
                if (!$this->reservationsScheduledForDeletion->isEmpty()) {
                    \Db\Example\ReservationQuery::create()
    ...
    protected function doInsert(ConnectionInterface $con): void
    {
        ...
        if ($this->isColumnModified(ResourceTableMap::COL_UID)) {
    ...
    public function buildCriteria(): Criteria
    {
        ...
        if ($this->isColumnModified(ResourceTableMap::COL_UID)) {
            $criteria->add(ResourceTableMap::COL_UID, $this->uid);
        }
        if ($this->isColumnModified(ResourceTableMap::COL_NAME)) {
     ...
     * objects.
     *
     * @param object $copyObj An object of \Db\Example\Resource (or compatible) type.
     ...
     *
     * @param bool $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
     * @return \Db\Example\Resource Clone of current object.
     * @throws \Propel\Runtime\Exception\PropelException
     */
    ...
    public function initReservations(bool $overrideExisting = true): void
    {
        ...
        $this->collReservations->setModel('\Db\Example\Reservation');
    }
        ...
        if (null === $this->collReservations || null !== $criteria || $partial) {
            if ($this->isNew()) {
                ...
                if (null === $this->collReservations) {
                    ...
                    $collReservations->setModel('\Db\Example\Reservation');
    ...
    public function setReservations(Collection $reservations, ?ConnectionInterface $con = null)
    {
        ...
        foreach ($reservationsToDelete as $reservationRemoved) {
            $reservationRemoved->setResource(null);
        }
